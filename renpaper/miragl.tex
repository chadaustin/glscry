...
\subsection{A Brief Introduction to Our Syntax}

Before we continue with the discussion of the advantage is our language 
provides, let us first introduce the look and feel of the language with an example.
...
Modern GPUs have a programming model similar to a stream processor: there is 
a stream or array of data elements (vertices or fragments) and a function is 
applied across all of them.  This function is often called a kernel.  Since all of the 
...
why graphics processors these days are so efficient.  Existing shading languages 
have semantic model similar to C; that is, they have the concept of variables that 
can be modified and read from.  Therefore they have an ordering requirement on 
individual statements.  Consider the following, C-like code:
...
Notice that, in general, the semantics of C are such that the order functions are 
called is important, as they may have side effects that are order dependent.  
This, in the case of shading languages, unnecessarily complicates the process of 
compilation and static analysis.  The high-level shading languages discussed 
above to a good job of analyzing execution flow and data dependency, so trivial 
examples such as these would not gain much, if any benefit.  However, shaders 
are growing in complexity as the hardware and support for effectively unlimited 
temporary use and instructions.  Eventually we will reach a point where the 
complexity of the shader seriously prohibits good optimization.  Pure functional 
languages, on the other hand, have very straightforward and restricted 
semantics.  In a modern functional language, and there are no mutable variables.  
Every function takes a set of values and returns another one, nothing else.  If you 
collect function twice with the same arguments, you always get the same night 
back.  Also, it does not matter which order you evaluate functions in.  This is 
called referential transparency, and a greatly improve the ability of both the 
compiler and programmer to reason about the computation, facilitating efficient 
generating code.

We believe the concept of referential transparency is a better fit for extreme 
programming model because it makes explicit that nothing can be modified, just 
returned.  This eliminates a potential source of confusion: the OpenGL shading 
language specification has to explicitly note that if you have a global variable and 
you modify it, that modification is only applicable within one stream element.


\subsection{Simplified Syntax w/ Type Inference}
...
skinnedVertex = sum [(weights i) * (bones (boneIndices i)) 
* gl_Vertex)
                     for i in (range 0 3)]
...
and GLSL, we decided to blur and abstract the difference between vertex 
shaders and fragment shaders.  One concern is that the different processors 
support different functionality, and by making the programs separate explicitly, 
the differences are made clear.  However, DirectX 10 is pushing for a unified 
processor architecture on both the fragment and pixel sides.  ATI technology has 
also recently announced that they are unifying their two processors.  With this in 
mind, we feel the potential confusion caused by executing "one" program on two 
processors (not counting the CPU) is worth the benefit in readability, 
maintainability, and optimization.

To mitigate the potential confusion by this approach, we provide a way to 
explicitly specify the frequency that computation runs at, as types can be made 
explicit even with the availability of type inference.


\subsection{Shaders As Data Files}

Following the example set by Cg and GLSL, it is key that shaders can be treated 
as data files so that they can travel along with the models whose surfaces they 
describe.  Requiring a compilation step before the ability to load them into a 
program increases the amount of time it takes to iterate changes, especially for 
people who aren't programmers.  For this reason, the approach taken by the 
metaprogramming shading systems, Sh and Vertigo, is infeasible.  The 
advantage of being able to use a fully featured general-purpose language for 
generation of your shaders is offset by the requirement of having a complete C++ 
or Haskell compiler in order to use your shaders and all.  That said, the basis of 
functional programming languages, the lambda calculus, has been shown to 
provide a very high degree of abstraction and notational convenience even with a 
very simple and naïve implementation.  Therefore, we can provide a high level of 
abstraction and performance with a minimum of effort, compared to an optimizing 
C compiler.
